<template>
	<scroll-view class="mdiv" style="padding: 15px;box-sizing: border-box;">
		...
		<div class="leaderboard" style="margin-right: 15px;">
			<header>
				...
				<h1 class="leaderboard__title"><span class="leaderboard__title--top">抢单达人</span><span class="leaderboard__title--bottom"></span></h1>
			</header>

			<main class="leaderboard__profiles">
				<article v-for="(item,index) in userArr" :key="index" class="leaderboard__profile">
					<img src="static/rank/normal.png" class="leaderboard__picture">
					<span class="leaderboard__name">{{strToStar(item.wx)}}</span>
					<span class="leaderboard__value">{{item.count_order}}</span>
				</article>
		...
		<div class="leaderboard" >
			<header style="background-image: linear-gradient(to right, #bdfac3, #db9fc6);">
...
C211 300 186 274 156 274" />
				...
				<h1 class="leaderboard__title"><span class="leaderboard__title--top" style="color:#fa335e">收益排行榜</span><span class="leaderboard__title--bottom"></span></h1>
			</header>

			<main class="leaderboard__profiles">
				<article v-for="(item,index) in userArr2" :key="index" class="leaderboard__profile">
					<img src="static/rank/first.png" class="leaderboard__picture">
					<span class="leaderboard__name">{{strToStar(item.wx)}}</span>
					<span style="font-size: 24px;color: rgba(0, 0, 0, 0.45)">￥<span class="leaderboard__value">{{item.balance/100}}</span></span>
				</article>
			...
			getSort() {
				db.collection("user-sort")
					...
					.then((res) => {
						...
						this.userArr.push(...res.result.data)
				...
				db.collection("user-sort")
					...
					.then((res) => {
						...
						this.userArr2.push(...res.result.data)
