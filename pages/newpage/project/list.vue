				...
				<uni-table :loading="loading" :emptyText="error.message || '没有更多数据'" border stripe type="selection"
				 @selection-change="selectionChange">
					<uni-tr>
						...
						<uni-th align="center">抢购价格(元)</uni-th>
						<uni-th align="center">回收价格(元)</uni-th>
					...
					<uni-tr v-for="(item,index) in data" :key="index">
						...
						<uni-td align="center">{{item.soldprice/100}}</uni-td>
						<uni-td align="center">{{item.buyprice/100}}</uni-td>
...
<script>
	...
	const dbSearchFields = ["no","name","desc"] // 支持模糊搜索的字段列表
	// 分页配置
	const pageSize = 20
	const pageCurrent = 1
