	...
	<view>
		<view class="uni-header">
			<view class="uni-group">
				<view class="uni-title">收货信息列表</view>
			 ...
			 :getcount="true" :page-size="options.pageSize" :page-current="options.pageCurrent" v-slot:default="{data,pagination,loading,error}">
				<uni-table :loading="loading" :emptyText="error.message || '没有更多数据'" border stripe type="selection"
				 @selection-change="selectionChange">
					<uni-tr>
						<uni-th align="center">收件人</uni-th>
						<uni-th align="center">收件地址</uni-th>
					...
					<uni-tr v-for="(item,index) in data" :key="index">
						...
						<uni-td align="center">{{item.addr}}</uni-td>
...
<script>
	...
	const dbSearchFields = ["name","addr","phone","comment"] // 支持模糊搜索的字段列表
	// 分页配置
	const pageSize = 20
	const pageCurrent = 1
