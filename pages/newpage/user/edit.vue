	...
	<view class="uni-container">
		<uni-forms ref="form" v-model="formData" :rules="rules" validateTrigger="bind" @submit="submit">
			...
			<uni-forms-item name="comment" label="团队名称">
				<input placeholder="团队名称" @input="binddata('comment', $event.detail.value)" class="uni-input-border" :value="formData.comment" />
			</uni-forms-item>
	...
	export default {
		data() {
			return {
				...
				rules: {
					...getValidator([  "nickname", "mobile","wx_unionid","comment"]),
					"status": {
						"rules": [{
							"format": "bool"
						}]
		...
		methods: {
			...
			getDetail(id) {
				...
				.doc(id)
				.field('nickname,mobile,wx_unionid,comment,status')
				.get()
				.then((res) => {
